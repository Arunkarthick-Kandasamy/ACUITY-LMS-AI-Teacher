...
export function LinkStudentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await apiRequest<{ link_id: string; student_id: string; full_name: string; status: string; message: string }>('/api/v1/parents/link', {
        method: 'POST',
        body: JSON.stringify({
          ...
          {success.status === 'pending' ? (
            <>
              ...
              <p className="text-sm text-slate-600 mb-6">
                Your link request has been sent to <strong>{success.full_name}</strong>.
                They need to <strong>approve</strong> the request from their Parent Access settings
            ...
            <>
              ...
              <p className="text-sm text-slate-600 mb-6">
                You are now linked to <strong>{success.full_name}</strong>. You can view their progress and reports.
              </p>
            </>
          )}
          <div className="space-y-2">
            <button onClick={() => navigate('/parent/dashboard')} className="btn-primary w-full">
              Go to Dashboard
            </button>
            <button onClick={() => { setSuccess(null); setCode(''); setParentEmail('') }} className="w-full py-2.5 border border-slate-300 text-slate-700 rounded-xl hover:bg-slate-50 transition-all text-sm font-medium">
    ...
    <div className="max-w-lg mx-auto mt-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Enter the 8-character code from your student's Parent Access settings to request linking.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 text-lg font-mono text-center tracking-widest rounded-xl border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-navy-800/20 focus:border-navy-800"
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-navy-800/20 focus:border-navy-800"
              placeholder="parent@example.com"
              autoComplete="email"
            />
            <p className="text-[10px] text-slate-400 mt-1">
              Your email helps the student identify who is requesting access.
            </p>
          </div>
