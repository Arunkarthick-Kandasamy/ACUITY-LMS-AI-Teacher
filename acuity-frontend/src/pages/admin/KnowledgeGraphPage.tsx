...
export function KnowledgeGraphPage() {
  ...
  useEffect(() => {
    const dbConcepts = localDb as any
    const conceptsData = mockConcepts.length > 0 ? mockConcepts : []
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          ...
          <div className="relative mb-3">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all" placeholder="Search concepts..." />
          </div>
          {loading ? (
            ...
            <div className="space-y-1 max-h-96 overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                filtered.map(c => (
                  <button key={c.concept_id} onClick={() => handleSelect(c.concept_id)} className={cn('w-full text-left px-3 py-2 rounded-lg text-sm transition-all', selectedConcept === c.concept_id ? 'bg-gray-900 text-white' : 'hover:bg-gray-100 text-gray-700')}>
                    <span className="text-xs text-gray-400 mr-2">#{c.order_index}</span>{c.title}
                  </button>
                ))
              )}
            </div>
          ...
          {selectedConcept ? (
            <>
              <div className="flex gap-2 mb-4">
                <input type="text" value={prereqId} onChange={e => setPrereqId(e.target.value)} className="flex-1 px-3.5 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all" placeholder="Prerequisite concept ID..." />
                <button onClick={addEdge} disabled={!prereqId} className="inline-flex items-center gap-2 px-3 py-2.5 bg-gray-900 text-white rounded-lg hover:bg-gray-800 disabled:opacity-50 transition-all text-sm"><Plus className="w-4 h-4" /></button>
              </div>
                ...
                {(prereqs.get(selectedConcept) || []).length === 0 ? (
                  ...
                  prereqs.get(selectedConcept)?.map((e: PrerequisiteEdge) => (
                    <div key={e.edge_id || e.prerequisite_id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <button onClick={() => deleteEdge(e.edge_id)} className="text-red-400 hover:text-red-600 transition-colors p-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="flex items-center justify-center py-12"><p className="text-sm text-gray-400">Select a concept to manage its prerequisites.</p></div>
          )}
        </div>
      </div>
    </div>
