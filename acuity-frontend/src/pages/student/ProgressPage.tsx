...
import { getScoreColor, getTrackLabel, cn } from '@/lib/utils'
...
export function ProgressPage() {
  ...
  const historyScores = masterySummary?.concepts
    ?.map(c => ({ lesson: c.concept_title?.split(' ').slice(0, 2).join(' ') || c.concept_id.slice(0, 4), score: Math.round(c.mastery_level * 100) }))
    || []
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {historyScores.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h2 className="font-semibold text-slate-900 mb-4">Concept Mastery</h2>
