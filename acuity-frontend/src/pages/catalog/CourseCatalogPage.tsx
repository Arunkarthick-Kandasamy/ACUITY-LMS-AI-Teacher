...
import { GraduationCap, Search, Clock, BookOpen, Layers, ArrowRight, ChevronLeft, ChevronRight, User } from 'lucide-react'
...
export function CourseCatalogPage() {
  ...
  const fetchCourses = useCallback(async (q: string, p: number) => {
    setLoading(true)
    try {
      const params = new URLSearchParams({ page: String(p), per_page: String(perPage) })
      if (q) params.set('search', q)
      const res = await apiRequest<{ items: CatalogCourse[]; total: number }>(`/api/v1/catalog?${params}`)
    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white">
      ...
      <section className="max-w-6xl mx-auto px-4 pb-8">
        <form onSubmit={handleSearch} className="flex gap-3">
          ...
          <button type="submit" className="px-5 py-2.5 bg-navy-800 text-white rounded-xl hover:bg-navy-700 transition-all text-sm font-medium shadow-sm">
            Search
          </button>
        </form>
      </section>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course) => (
                <div
                  ...
                  className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-all flex flex-col"
                >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-navy-800 text-white rounded-lg hover:bg-navy-700 transition-all text-sm font-medium shadow-sm"
                  >
                    Start Learning
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                <button
                  ...
                  disabled={page === 1}
                  className="p-2 rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 disabled:opacity-30 disabled:pointer-events-none"
                >
