      ...
      <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
            ...
            <p className="text-sm text-slate-600 mb-6">If an account exists for <strong>{email}</strong>, we've sent a password reset link.</p>
            <button onClick={() => navigate('/login')} className="text-sm text-navy-700 hover:text-navy-900 font-medium">← Back to sign in</button>
          </div>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-navy-800/20 focus:border-navy-800" placeholder="you@example.com" required />
              </div>
            </div>
        ...
        <p className="text-center mt-6">
          <button onClick={() => navigate('/login')} className="inline-flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600"><ArrowLeft className="w-3 h-3" /> Back to sign in</button>
        </p>
      </div>
    </div>
  )
